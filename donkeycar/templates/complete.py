...
def drive(cfg, model_path=None, use_joystick=False, model_type=None, camera_type='single', meta=[], port=None, simpath=None):
    ...
    if use_joystick or cfg.USE_JOYSTICK_AS_DEFAULT:
        ...
        ctr = LocalWebController(port=port, mode=cfg.WEB_INIT_MODE)
    ...
    if type(ctr) is LocalWebController:
        if cfg.DONKEY_GYM:
            print("You can now go to http://localhost:%d to drive your car." % port)
        else:
            print("You can now go to <your hostname.local>:%d to drive your car." % port)
